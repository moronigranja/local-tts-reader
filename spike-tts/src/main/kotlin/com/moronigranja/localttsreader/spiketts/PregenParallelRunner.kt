 ...
 */
class PregenParallelRunner(private val context: Context) {
    ...
    private data class Passage(val text: String, val language: String, val voice: String, val phonemes: String)

    /** One synthesized passage (PCM kept for the oracle). */
    private data class Synth(val passage: Passage, val millis: Long, val seconds: Double, val pcm: FloatArray)
    ...
    fun run(log: (String) -> Unit): Boolean {
        return try {
            ...
            val passages = corpusFile.readLines().mapNotNull { line ->
    ...
    private suspend fun serialLeg(passages: List<Passage>, log: (String) -> Unit): SerialLeg {
        ...
        for (run in 1..RUNS) {
            ...
            log("serial run $run/$RUNS: ${"%.2f".format(audioSeconds)}s audio in $runWall ms, " +
        ...
        var rrMeanSum = 0.0
        var rrCount = 0
        if (run1 != null) for (p in passages) {
            val a = run1[p.text]
    ...
    private suspend fun parallelLeg(
        ...
        val tOpen = System.currentTimeMillis()
        val engineA = KokoroEngine.open(
            spec = DefaultEngines.kokoro, packs = KokoroPacks.all,
            modelFile = File(models, "kokoro-model"), voicesFile = File(models, "kokoro-voices"),
            phonemizer = CorpusPhonemizer(passages),
            progress = { log("parallel open A: $it (${System.currentTimeMillis() - tOpen} ms)") },
            sessionFactory = {},
        )
        val engineBOpen = System.currentTimeMillis()
        val engineB = KokoroEngine.open(
            spec = DefaultEngines.kokoro, packs = KokoroPacks.all,
            modelFile = File(models, "kokoro-model"), voicesFile = File(models, "kokoro-voices"),
            phonemizer = CorpusPhonemizer(passages),
        ...
        for (run in 1..RUNS) {
            ...
            val runWall = measureTimeMillis {
                val results = coroutineScope {
                    val evens = async(Dispatchers.IO) {
                        synthesizeAll(engineA, passages.filterIndexed { i, _ -> i % 2 == 0 })
                    }
                    val odds = async(Dispatchers.IO) {
                ...
                for (s in results) {
                    ...
                    if (oraclePcm != null) {
                        ...
                        if (peak > worstPeak) { worstPeak = peak; worst = s; worstSerial = oraclePcm }
                        meanAbsDiffSum += mean
                        diffCount++
                        log("  parallel[run $run] [${s.passage.language}] peak=${"%.6f".format(peak)} mean=${"%.6f".format(mean)} lenDelta=${s.pcm.size - oraclePcm.size}")
                    }
                }
                audioSeconds = audio
            }
            log("parallel run $run/$RUNS: ${"%.2f".format(audioSeconds)}s audio in $runWall ms " +
    ...
    private fun synthesizeAll(engine: KokoroEngine, passages: List<Passage>): List<Synth> {
    ...
    private class CorpusPhonemizer(private val passages: List<Passage>) : Phonemizer {
