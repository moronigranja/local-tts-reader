...
core-player/      LIVE — v1 player state machine (T4-1, #33): transport, transactional writes, undo ring, sleep timer, speed, bookmarks; PlayerStore contract; T5 pre-generation (PregenQueue + PcmPassageCache + OfflinePregen planner, #35/#42)
...
feature-library/  SAF import + library list UI (Compose, Hilt) — C5/C6; search pending; library-row pre-gen action over feature-player's PregenManager (#42)
feature-player/   LIVE — playback service, MediaSession, pregen wiring (queue + disk tier, #35/#42); reader surface + S3 gestures (#38); PregenWorker/PregenManager overnight + manual offline pre-gen (#42)
