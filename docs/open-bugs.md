...
| **Reader shows the empty-state placeholder after screen off/on (intermittent)** | After turning the display off and back on, the reader sometimes shows "Tap play to start listening from this book." — the empty branch `chapterPassages.isEmpty() && phase == PlayerPhase.IDLE` (ReaderScreen) — instead of the book text; the reading place is gone. Intermittent, not reliably reproducible: a quick off/on with the app process alive does NOT trigger it (S22 2026-09-07, process survived). Mechanism family: the reader's `LaunchedEffect(bookId, startAt)` only re-opens when `bookId` changes or `startAt != null`, so a service-state reset or process death underneath (`PlaybackStateHolder.reset()` / machine teardown — e.g. LMK while the screen is off) leaves the reader on the empty branch with no recovery. | Open — investigate later: confirm state-reset vs process-death, then re-open (or restore from the persisted resume row) when the state underneath changes | user report 2026-09-07 |
