...
| **PlaybackE2eTest/PregenE2eTest device failures (pre-existing fixture geometry)** | `PlaybackE2eTest.playsThroughTheBookAndCompletes` times out: the 90 s test window is consumed by the 60 s `PLAY_BUFFER_TIMEOUT_MS` wait on a 2-passage book that can never reach `PREFILL_LOOKAHEAD_SECONDS = 45` ahead. `PregenE2eTest.workerPregensTheBookThenPlaybackCompletesOverTheCache` can flake: the worker reads the shared Room DB via the app singleton and can see the fixture book missing (WAL/instance race), settling SUCCESS with an empty cache. Not regressions of the immersive plan; surfaced during its device pass. | Open — suggest widening the PlaybackE2E window (or pinning the buffer wait for the fixture) and re-opening the store singleton in PregenE2E setUp | device run 2026-09-04; items 1-8 pass |
