...
| **Room DB absent for hours after reinstall, then an empty file appeared; library rendered books from non-Room state** | S22 observation 2026-08-29 (after `adb install -r` of a new build at 13:03): `run-as` showed NO `files/databases/` at 17:44 and 17:46 (`find . -name "*.db*"` → only `cache/local-tts-reader.db.lck`), yet the library rendered three "Continue listening" cards (A Deepness in the Sky 1% / 53.2 MB offline, E2E Test Book 100%, Pregen E2E Book 100%) whose resume rows normally come from Room. Play taps on those cards produced NO `PlaybackService` logs and NO MediaSession (playback dead — command targets book ids no durable store holds). By ~18:05 the DB existed but pulled as a **68 B empty Room skeleton** (no tables; sqlite "file is not a database"). The pregen tier still held the books' keys (`files/pregen/t4-e2e-book`, `files/pregen/3a9c5f…`) — derived audio outliving the durable store, the inverse of the CR-3 durability ordering. **Code trace done (2026-08-29, decisions #88):** `LibraryViewModel` sources `library`/`recent`/`readProgress` exclusively from Room DAOs (`repository.books`, `progressDao.observeAll()`, `passageDao.chapterCounts()`); the only `LibraryStore` binding is `RoomLibraryStore`; the provider is a standard `Room.databaseBuilder` with `MIGRATION_1_2`; the DB is first opened at app start by the `LocalTtsReaderApp` index rebuild. There is NO non-Room book path in the app — the cards' render provably required an open, valid database, so the file absence at 17:44 was a filesystem/data-dir layer event (Samsung install-time data handling), not an app data-flow bug; the FS-side deletion mechanism was not reproduced on the host. **App-side fragility shipped (decisions #88):** a corrupt/truncated file at the db path now crashes every launch (Room throws at first DAO access); `CorruptDatabaseGuard` quarantines it (moves main + `-wal`/`-shm` to `files/corrupt-db/<ts>/`, preserved for analysis) before the builder opens, so a garbage file yields a recoverable empty library instead of a crash loop. Host-tested (`CorruptDatabaseGuardTest` ×4). FS-side root cause remains open | device pass 2026-08-29, S22 after `install -r` |

> 2026-08-31: the FS-side root cause of the row above now has an owning roadmap
slice — **A8** (the app-side trace and the `CorruptDatabaseGuard` mitigation
already shipped in decisions #88): reproduce the deletion, classify Samsung
install-time handling vs an app trigger, fix or close with evidence; gates
Phase C (decisions #96).
