...
| **Pocket TTS** (Kyutai; [tech report 2026-01-13](https://kyutai.org/blog/2026-01-13-pocket-tts), [paper](https://arxiv.org/abs/2509.06926)) | MIT code / **CC-BY-4.0 weights**; 100M params; dual-model flow-matching split (`flow_lm_main` conditioner + `flow_lm_flow` Euler step) + `text_conditioner` + Mimi encoder/decoder = **5 ONNX graphs**, int8 variants for the flow LM and decoder (Mimi encoder/text conditioner stay fp32); en (`english_2026-04` 6-layer), de, fr, it, pt, es (+ `*_24l` higher-quality/slower bundles for all but en); zero-shot cloning (Mimi encoder → speaker latents); streaming with adaptive chunking (starts at 2 frames); `lsd_steps` and `temperature` dials | Vendor: **6.33× realtime on an M4 CPU using 2 cores**, ~200 ms to first chunk; int8 7.76× (ARM/torchao) and 4.04× (x86/CI); no phone RTF published anywhere | **Reopened 2026-09-11 — the "no Android runtime path" veto was wrong.** NekoSpeak v1.4.2 ships it as its *recommended* engine: the five ONNX graphs through ORT sessions, int8 on the heavy ones, behind a device-speed-adaptive streaming buffer (probes §"Cross-app on-device Kokoro survey"). Community exports exist and are pinnable — `KevinAHM/pocket-tts-onnx` (int8 + streaming, per-language bundles), `lookbe/pocket-tts-onnx`, plus a C++ ORT runtime `VolgaGerm/PocketTTS.cpp`. **D5-relevant**: a 100M-class *cloning* engine at ~176 MB against CosyVoice3's 3.47 GiB and Chatterbox's 3.2 GB — ~20× smaller, i.e. the first candidate that could put cloning inside a phone's live budget rather than pregen-only. Open gates before it is a leg: no ARM/phone RTF exists (AR loop + flow steps + Mimi decode unmeasured); **Misaki/sentencepiece tokenizer = a second G2P convention** beside espeak-ng (#97's one-convention rule); read-along timing unverified (Mimi frames are 12.5 Hz — coarse, not word-level, so #30b applies as with Piper); **license audit (2026-09-11, #149)**: code MIT, weights CC-BY-4.0 but `kyutai/pocket-tts` is **gated-auto** with prohibited-use terms (cloning/impersonation without consent), so a token-less app cannot fetch the upstream weights — the ungated CC-BY-4.0 export (`KevinAHM/pocket-tts-onnx` @ `58a6d00c…`) is the fetchable path; the voice repo is mixed-license (`voice-donations/` + `voice-zero/` CC0; `vctk/`, `alba-mackenna/`, `cml-tts/fr/` CC-BY-4.0; **`expresso/` + `ears/` CC-BY-NC — excluded**, and the model repo's built-in embeddings are derived from them, so a permissive-only catalog must be filtered, not inherited) |
...
| **NekoSpeak** (`siva-sub/NekoSpeak`, MIT; v1.4.2) | ORT 1.18 Kotlin AAR, CPU EP only | `kokoro-v1.0.int8.onnx` 92,361,271 B dynamic QUInt8 (on-demand ~115 MB); Piper Amy Low bundled; **Pocket TTS ~176 MB on-demand — the author's *recommended* engine** | ~150-token window batching + **first-sentence immediate flush** (TTFA); chunk-length-matched style vector; engine-swap service shape; and it **runs Pocket TTS as five ORT sessions** (Mimi encoder/decoder, text conditioner, `flow_lm_main`/`flow_lm_flow`) with int8 on the heavy graphs, a pure-Kotlin Misaki G2P (Viterbi heteronym resolution), and a **device-speed-adaptive streaming buffer** (measures frame time against the ~80 ms frame budget, then sizes 8/10/15/20 frames) — the Android runtime proof for the Pocket TTS reopening in §"D3 comparison sweep (2026-08-29)" |
...
| **kokoro-reader** (`Samhamsam/kokoro-reader`, MIT; owner probe 2026-09-11) | **No on-device neural path**: the desktop Rust app is a PDF reader + HTTP client (its `Cargo.toml` has no `ort`/`tract`/`candle`) and the Android app ships the **system TTS** (`TtsEngine.kt`) plus a `ServerTtsEngine.kt` that POSTs to a Go server running **sherpa-onnx** (`NumThreads = 4`, espeak-ng data dir) | Server-side `kokoro-en` (model.onnx + voices.bin) and `piper-thorsten` (`de_DE-thorsten-high.onnx`) — but `engine.go` hardcodes exactly **7 voices** (6 EN Kokoro speakers + 1 DE Piper), not the README's "50+ voices". Both model dirs are symlinks out of the repo | Delegates synthesis off-device: `GET /api/voices` + **one HTTP POST per sentence** (10 s connect/30 s read, 3 consecutive failures → `ERROR`), bounded 5-chunk audio queue, no cache, no read-along anchors — a working miniature of the LAN-server shape the companion idea contemplates, with the streaming variant's failure mode on display. Nothing to take on performance: same ORT/sherpa kernels, no published RTF. Its sentence splitter + TTS-only text prep (`, (` around brackets, `), ` after, standalone page-number lines dropped) are G1 inputs, and its German Piper pin is a D4 candidate (both recorded there). The Rust work it credits is real but lives elsewhere: `8b-is/kokoro-tiny` (Apache-2.0, crates.io, **ORT + espeak-ng like us**, weighted *voice mixing* via style vectors, streaming) and a crate ecosystem (`kokoro-en`, `kokoro-micro`, `hematite-kokoros`, `any-tts`) — all the same 82M export on ORT or Candle, so still no speed lever |
...
[kokoro-reader](https://github.com/Samhamsam/kokoro-reader) (code, not the README — the claims
and the engine disagree; its credit line's `8h-is/kokoro-tiny` is a **stale handle**, not a dead
project — the repo is [`8b-is/kokoro-tiny`](https://github.com/8b-is/kokoro-tiny), Apache-2.0,
`pushed 2026-05-02`, ORT + espeak-ng like us, with a wider Rust crate ecosystem around it
(`kokoro-en`, `kokoro-micro` (a live derivative, pushed 2026-09-09), `hematite-kokoros`,
`any-tts` — all the same 82M export on ORT or Candle, none a speed lever) ·
[kokoroxide](https://github.com/dhruv304c2/kokoroxide) (the actual Rust ONNX port: `ort` +
espeak-ng → Misaki-token mapping, en-only, Apache-2.0/MIT) ·
