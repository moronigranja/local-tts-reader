...
| Offline translation to a target language (read a book aloud in another language — e.g. it→es, en→pt) | owner (2026-08-25; feasibility + complexity reviewed; scope widened to any advertised target language 2026-08-31, decisions #101) | Kokoro v1.0 already ships the advertised es/it/pt voices; ONNX Runtime already in the stack; a `TTSEngine` decorator adds the stage without touching player/pipeline contracts | T5 + new `core-translate` — **promoted to roadmap "Later"** (2026-09-10) | NMT per language pair, int8 (~30–80 MB; OPUS-MT CC-BY-4.0 — attribution; NLLB-600M CC-BY-NC — avoid). "Real-time" = hidden behind the pre-gen queue, not simultaneous interpretation. Translation failure degrades to the original text; index/matching unaffected (output-side only). One pack + quality gate per pair; single multilingual model stays a design-time gate | Phase J spike measured, owner ADOPTED small100 int8 as the translate-then-read direction (2026-09-02, decisions #114): one pack for all languages (916 MB), ~1.06 GB leg PSS, 8.9–9.9 ms/tok decode, quality compromise accepted (chr-F 62.1 vs opus-tc-big 67.4 on en→pt-br). Per-pair OPUS-MT remains the measured record/alternative. Follow-up: core-translate slice (SMaLL-100 tokenizer port + pack integration behind pre-gen, output-side only).
...
| Pre-generate custom time amount (arbitrary duration, not fixed presets) | owner (2026-08-31) | The library-row "Pre-generate" dialog already collects a listening-time budget but ships fixed presets (30 m / 1 h / 2 h / 3 h / whole book); the backend already accepts any `PregenBudget.maxTimeMs` (A1 made bounded vs unbounded/exhausted distinct), so an arbitrary value is a UI-only change — parse input → same `pregenerate(bookId, budget)` | Pre-gen follow-up batch (decisions #44 family: space estimate, per-book usage/delete); `PregenBudgetDialog` surface | Manual offline budget, not the D1 live look-ahead horizon; budget is not part of the `PregenKey` cache key (engine+voice+speed+translation), so custom values need no cache migration |
...
| ~~Long-press paragraph menu — "Play from here" + "Copy text"~~ | owner (2026-08-27) | Middle-tap play-at-passage was superseded by the double-tap chrome toggle + play-from-visible-page-top (decisions #122; the long-press gesture was rejected originally as less discoverable — a menu is the discoverable superset); copy is nearly free: the passage text is already in `PlaybackUiState.passageText`, and tapped-line→passage mapping exists (`passageStartLines`/`indexOfLast`) | **Landed** — G2, 2026-09-06 (decisions #127); plan `docs/features/paragraph-context-menu.md` | Copy = modern suspend `Clipboard.setClipEntry` on the passage string, no schema; menu must not fight the middle double tap (long-press vs double-tap vs single-tap discrimination — per-down `longPressTimeoutMillis` deadline in the single `awaitEachGesture` loop); chapter-level copy ("Copy chapter") remains a possible menu extension …
...
| Chaptered audiobook export (`.m4b`) | candela: "make your own audiobook" — render any fiction to a chaptered `.m4b` through the chosen neural voice (per-chapter markers) | The hard half already exists: pre-generation renders whole books to disk PCM (`PregenQueue`/`PregenKey`); remaining work is an AAC/m4b muxer + chapter markers + a settings surface | Post-v1; offline-audio settings section | No new engine surface — rides the pregen pipeline; fits offline-first ("listen anywhere", share public-domain audio); cache-key rules of `PregenKey` apply unchanged |
| Desktop companion pre-generation producer (PC synthesizes, phone plays from cache) — **batch, explicitly not a live streaming server** | owner (2026-09-11) | #147 measured the drain this removes: live synthesis 3.0–4.8 W, ~10% of a 4800 mAh battery per listening *hour*, thermal status 0 → 3 (SKIN 36 → 45 °C) in ~15 min, and the doc's own conclusion that cache playback is the only lever that idles the SoC while listening (thread tuning is not a rescue — 1 thread is slower than realtime at the worst energy per audio hour). Host RTF is **0.15–0.23** (Ryzen 9 8945HS, fp32, ORT JVM — decisions #26) against the Fold 8's 0.575 @4T (#147) and HiBreak 2.84–3.12 (#99): a whole book in ~1.5–3 h of background PC work. The **cache is already the interface** — `PregenKey` path `<bookId>/<engine>/<voice>/<speed>/c<ch>p<passage>` + `.meta` sidecar (sample rate + sentence anchors), raw PCM16, and `PcmPassageCache.bootstrap()` adopts any valid entry under the root, so foreign audio plays through the unchanged `get()` → AudioTrack path (no player, Room or pregen-queue change). `core-*` is pure JVM by construction (§architecture 1) — Kokoro + espeak-ng JNA (desktop natives are the normal case; the Android AAR was the special one), parse → segment, cache writer — so a companion head reuses the same code instead of porting the pipeline. Batch suits the product: an audiobook only needs generation ahead of the playhead (hard-facts allows non-realtime synthesis), whereas a live socket in the happy path violates offline-first | Post-v1, pooled — new desktop/JVM head + a SAF ingest edge; no roadmap slot | **No user-reachable ingest exists today**: the cache root is `filesDir/pregen`, so this needs a SAF import edge (copy tree → `bootstrap()` adopts it, zero cache changes) or an optional pregenerated-audio section in the E1 backup codec (#111) reusing the existing SAF transport. **Parity hazard (sharpest)**: `PregenKey` carries no model or phonemizer revision, so a different espeak-ng build (phone pack is pinned, #32), a quantized model, or drifted trim/pause math writes audio the phone treats as correct and cannot distinguish — reuse the same core + SHA-pinned packs (#23/#28), never a reimplementation. **Transfer volume**: ~170 MB/h PCM (≈1.4 GB per 8 h book) against the 4 GiB LRU cap; Opus stays a cache-format dead end (#46/#50 — MediaCodec decode broken on the S22, libopus rejected), so any compression is a *transfer* codec, never a cache format. **Quality comes from engine class, not PC speed** (same weights sound identical): the payoff is running the pregen-only tier — CosyVoice3 RTF 12.5–31.1 on the S22 (#21/#49/#93), MOSS lmkd-killed at ~2.5 GB on the HiBreak — which is the delivery path for the roadmap's high-end cloned-voice row and takes the phone's memory/thermal axis out of D5. Complementary to D7 (which optimizes the phone-only path), not a competitor; only helps users with a PC + sync habit, so it stays an optional producer, never load-bearing. Measurement gate before any code, per the planning rules: host wall-clock per book, a real ingest round trip, and a parity check that companion-written entries are adopted by `bootstrap()` and play with zero resynthesis |
## Validated, no action
...
## Decision status (decisions #29 snapshot 2026-08-25; updated 2026-09-11)
...
| Desktop companion pre-generation producer | Stays in the pool (2026-09-11): batch pregen offload only — the cache is the interface and `bootstrap()` already adopts foreign entries, but the ingest edge (SAF import or a backup-codec section) and the model/phonemizer parity rule are undecided; explicitly **not** a live streaming server |
