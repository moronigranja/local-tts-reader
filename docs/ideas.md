...
| Background import with progress notification | owner (2026-08-25, import review) | Import runs in `viewModelScope` on `Dispatchers.IO` — cancelled on process death, progress is UI-only (invisible when backgrounded); a long folder import deserves a notification the user can watch and leave | feature-library follow-up (small slice) | Persistable URI grants are already taken at pick time (`takePersistableUriPermission`), so a job can re-read files after restart; foreground service (`dataSync`) = live ticks + process-safe; WorkManager = survives death, slower feedback; must not collide with the T4 playback foreground service |
## Validated, no action
