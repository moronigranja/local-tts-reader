...
| Long-press paragraph menu — "Play from here" + "Copy text" | owner (2026-08-27) | Middle-tap play-at-passage was superseded by the double-tap chrome toggle + play-from-visible-page-top (decisions #122; the long-press gesture was rejected originally as less discoverable — a menu is the discoverable superset); copy is nearly free: the passage text is already in `PlaybackUiState.passageText`, and tapped-line→passage mapping exists (`passageStartLines`/`indexOfLast`) | feature-reader (small slice; #38 deviation note) | Copy = `ClipboardManager` on the passage string, no schema; menu must not fight the middle double tap (long-press vs double-tap vs single-tap discrimination); optionally extend to chapter-level copy ("Copy chapter") once the context menu exists |
