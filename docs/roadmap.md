...
| T3 | **CosyVoice3 spike**: verify community ONNX export on S22 Ultra; measure RTF/RAM/thermal → engine-order decision | 2–4 d | Gate result: decisions #21 — CPU fails (~RTF 16–22), flow DiT is the wall; audio-fidelity follow-up closed (#21), re-run clean on S22 (#49) |
...
- **App-wide player card — SHIPPED + verified 2026-08-27 (decisions #55,
  refined by #56).** Library: the card replaces the top Continue-list row in
  place (expand animation), with the row's overflow + offline badge; reader:
  docked card, controls only. Chapter skip cut to core (#56). Follow-up:
  time-bounded look-ahead + survive-seek ensure for instant ±30s.
