...
class ImportCoordinatorTest {
    ...
    private fun coordinator(store: LibraryStore = InMemoryLibraryStore(), index: TextIndex = TextIndex()) =
        ImportCoordinator(BookImporter(), store, index, IndexLock())

    /** Flat cache rows the rebuild consumes (the Room mapper's shape). */
    private fun cached(entry: LibraryEntry): CachedBook = CachedBook(
        id = entry.book.id,
        title = entry.book.title,
        authors = entry.book.authors,
        passages = entry.book.chapters.flatMap { ch ->
    ...
    private class FlakyStore(private val inner: InMemoryLibraryStore = InMemoryLibraryStore()) : LibraryStore {
        var failures = 0
    ...
    @Test
    fun `import commits durably then publishes to the index`() = runTest {
    ...
    @Test
    fun `the durable store is the duplicate gate - no re-parse for an existing book`() = runTest {
        val store = InMemoryLibraryStore()
    ...
    @Test
    fun `a failed durable commit never poisons the retry`() = runTest {
        val index = TextIndex()
        ...
        assertEquals(1, index.bookCount())
        val id = flaky.books.value.first().book.id
    ...
    @Test
    fun `a stale rebuild snapshot can never purge a concurrently committed book`() = runTest {
        val store = InMemoryLibraryStore()
        val index = TextIndex()
        val lock = IndexLock()
        ...
        val rebuild = launch(StandardTestDispatcher(testScheduler)) {
        ...
        val import = launch(StandardTestDispatcher(testScheduler)) {
        ...
        assertEquals(setOf(idA, store.books.value.first { it.book.id != idA }.book.id), index.bookIds().toSet())
    }
    ...
    @Test
    fun `batch import reports progress around every file`() = runTest {
        val progress = mutableListOf<Triple<String, Int, Int>>()
        val outcomes = coordinator().importAll(
            listOf(
