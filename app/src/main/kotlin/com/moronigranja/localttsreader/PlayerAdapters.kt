 ...
 */
@Singleton
class PlaybackCommandSender @Inject constructor(
    ...
    override fun play(bookId: String) = command(PlaybackService.ACTION_PLAY) {
...
@Singleton
class WorkManagerPregenScheduler @Inject constructor(
    private val manager: PregenManager,
) : PregenScheduler {

    override fun pregenerate(bookId: String, budgetMinutes: Long?) =
        manager.pregenerate(bookId, budgetMinutes)

    override fun cancel(bookId: String) = manager.cancel(bookId)

    override fun observe(bookId: String): Flow<PregenJobState> = callbackFlow {
        val live: LiveData<List<WorkInfo>> = manager.workInfo(bookId)
        val observer = Observer<List<WorkInfo>> { infos ->
            val settled = infos.lastOrNull {
            ...
            trySend(
                PregenJobState(
                    ...
                    error = if (settled?.state == WorkInfo.State.FAILED) {
